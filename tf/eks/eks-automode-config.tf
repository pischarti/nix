locals {
  ...
  config_path = "${path.module}/terraform-aws-eks-blueprints/patterns/eks-automode/automode-custom-nodepools/eks-automode-config"
}
...
resource "kubectl_manifest" "storageclass_yamls" {
  ...
  yaml_body = file("${local.config_path}/${each.value}")
}
...
resource "kubectl_manifest" "custom_nodeclass" {
  ...
  yaml_body = templatefile("${local.config_path}/${each.value}", {
